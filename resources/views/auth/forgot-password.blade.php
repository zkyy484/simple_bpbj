...
<head>
    ...
    <title>Lupa Password - Buku Tamu Digital Bagian PBJ</title>
...
<body class="bg-[#cddcfd] min-h-screen flex items-center justify-center p-4 font-['Poppins'] antialiased">
    ...
    <div class="w-full max-w-[440px] bg-white rounded-[24px] shadow-sm p-8 md:p-10 text-center">
        ...
        <!-- INFORMASI / INSTRUKSI -->
        <p class="text-xs text-gray-600 mb-6 text-left leading-relaxed">
        ...
        @if(session('status'))
            <div class="mb-5 rounded-xl bg-green-100 border border-green-300 text-green-700 px-4 py-3 text-xs text-left">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('password.email') }}" class="text-left">
            ...
            <div class="text-center">
                <a href="{{ route('login') }}" class="text-xs text-[#2f53d7] underline hover:text-[#1a38a7] transition-colors">
