...
@section('content')
    ...
    }" class="relative" :data-modal-open="openDetail || openDelete">
                ...
                <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                    ...
                    @if ($respons->hasPages())
                        <div
                            ...
                            <p class="text-xs text-gray-500">
                                Showing {{ $respons->firstItem() }} to {{ $respons->lastItem() }} of
                                {{ $respons->total() }} entries
                            </p>
                    ...
                    @else
                        <div class="px-6 py-4 border-t border-gray-100">
                            <p class="text-xs text-gray-500">
                                Showing {{ $respons->count() ? 1 : 0 }} to {{ $respons->count() }} of
                                {{ $respons->total() }} entries
                            </p>
                        </div>
                    @endif
...
@push('scripts')
    ...
    <script>
        ...
        // AUTO REFRESH (AJAX) - "DAFTAR RESPON SURVEI"
        (function() {
            const wrapper = document.getElementById('tabel-survei-wrapper');
            if (!wrapper) return;
            ...
            function refreshTable() {
                ...
                fetch(buildRefreshUrl(), {
                    ...
                    .then((res) => {
                        if (!res.ok) throw new Error('Gagal memuat data survei.');
                        return res.text();
                    })
                    .then((html) => {
                        const parser = new DOMParser();
                        const doc = parser.parseFromString(html, 'text/html');
                        const newWrapper = doc.getElementById('tabel-survei-wrapper');
