...
<script>
    ...
    (function () {
        const wrapper = document.getElementById('tabel-sub-bagian-wrapper');
        if (!wrapper) return;
        ...
        function refreshTable() {
            ...
            fetch(buildRefreshUrl(), {
                ...
                .then((res) => {
                    if (!res.ok) throw new Error('Gagal memuat data sub bagian.');
                    return res.text();
                })
                .then((html) => {
                    const parser = new DOMParser();
                    const doc = parser.parseFromString(html, 'text/html');
                    const newWrapper = doc.getElementById('tabel-sub-bagian-wrapper');
