        ...
        <div id="tabel-pengunjung-wrapper">
            <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    <h3 class="text-base font-bold text-gray-900">Daftar Laporan Pengunjung</h3>
                    <span class="text-xs bg-blue-50 text-blue-700 px-3 py-1 rounded-full font-semibold">
                        Total : {{ $pengunjungs->total() ?? 0 }}
                    </span>
                </div>
...
<script>
    ...
    (function () {
        const wrapper = document.getElementById('tabel-pengunjung-wrapper');
        if (!wrapper) return;
        ...
        function refreshTable() {
            ...
            fetch(buildRefreshUrl(), {
                ...
                .then((res) => {
                    if (!res.ok) throw new Error('Gagal memuat data laporan pengunjung.');
                    return res.text();
                })
                .then((html) => {
                    const parser = new DOMParser();
                    const doc = parser.parseFromString(html, 'text/html');
                    const newWrapper = doc.getElementById('tabel-pengunjung-wrapper');
