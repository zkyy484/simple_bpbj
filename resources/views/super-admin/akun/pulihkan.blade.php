    ...
    <div x-show="openRestore" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="p-6 text-center space-y-3">
            <p class="text-gray-600 text-sm">
                Apakah Anda yakin ingin memulihkan akun ini?
            </p>
            <div class="p-3 bg-emerald-50 rounded-xl border border-emerald-100 text-emerald-700 font-semibold text-sm">
                <span x-text="selectedAccount.nama"></span>
            </div>
        </div>

        <!-- Footer Form -->
        <form action="{{ route('super.akun.pulihkan') }}" method="POST">
            @csrf
            @method('PUT')

            <!-- Hidden input ID User -->
            <input type="hidden" name="id_user" x-model="selectedAccount.id">
